...
app.post("/add_to_cart", (req,res) => {
    ...
    var product = {
        id: id, name: name, price: price, sale_price: sale_price, quantity: quantity, image: image
    }
        ...
        if(err){
            ...
            con.query(query, [values], (err, result) => {
                for(let i=0; i<cart.length; i++){
                    ...
                    var values = [
                        [result.insertId, cart[i].id, cart[i].name, cart[i].price, cart[i].image, cart[i].quantity, new Date()]
                    ];
                    con.query(query, [values], (err, result) => {})
                }
...
app.get("/search_product", (req, res) => {
    ...
    con.query("SELECT id FROM products WHERE name = '"+productName+"'", (err, result) => {
        ...
        res.redirect(`/single_product?id=${productId}`);
    })
